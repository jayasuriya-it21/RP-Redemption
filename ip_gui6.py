...
def calculate_reward_points(*args):
    ...
    try:
        ...
        if subject_type in ["normal", "lab"] and 0 <= internal_marks <= (15 if subject_type == "normal" else 20):
            result = f"Reward points required: {calculate_reward_points_function(subject_type, internal_marks, redemption_ratio)}"
        else:
            result = "Invalid input: Check subject type and internal marks range"
    except ValueError:
        result = "Invalid input: Ensure marks and ratio are numbers"
...
def calculate_reward_points_function(subject_type, internal_marks, redemption_ratio):
    ...
    if subject_type == "normal":
        ...
        if internal_marks <= 10:
            ...
            points = (internal_marks - 18) * redemption_ratio * 6 + 2 * redemption_ratio * 4 + 4 * redemption_ratio * 2 + 10 * redemption_ratio
...
subject_type_var = tk.StringVar(value="Normal")
...
# Grid layout
subject_type_label.grid(row=0, column=0, padx=10, pady=5, sticky="w")
subject_type_combobox.grid(row=0, column=1, padx=10, pady=5, sticky="w")

internal_marks_label.grid(row=1, column=0, padx=10, pady=5, sticky="w")
internal_marks_entry.grid(row=1, column=1, padx=10, pady=5, sticky="w")

redemption_ratio_label.grid(row=2, column=0, padx=10, pady=5, sticky="w")
redemption_ratio_entry.grid(row=2, column=1, padx=10, pady=5, sticky="w")
